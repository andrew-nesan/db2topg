...
use Getopt::Long;
use Encode::Guess
  ; # No idea what encoding DB2 will use for its dump (for objects with diacritic characters)
...
# We have an exception for create functions: there will be semi-columns inside…
    ...
    # If the function contains a begin atomic, lets suppose it's sort of plpgsql
            ...
            if ( $line =~ /^(?:(.*): )?\s*begin(\s+atomic)\s*/i ) {
                if ( defined $1 ) {
                    $marker = " $1"
                      ; # We add a space before, it will simplify the regexp in the following test
                }
                else {
                    $marker = "";
                }
...
sub find_default_default {
    my ($type) = @_;
    if ( $type =~
        /^(SMALLINT|INT|BIGINT|DECIMAL|NUMERIC|REAL|DOUBLE|DECFLOAT|FLOAT)/i )
    {
        return 0;
    }
    ...
    croak
"Unknown type $type when trying to find 'default' default value for a type\n";
}
...
sub convert_type {
    ...
    if ( $in_type =~ /^BLOB\((\d+)\)/ ) {
        $out_type = "bytea"
          ; # Could add a check constraint to verify size, but most of the time, the size is here
    }
    elsif ( $in_type =~ /^CLOB\((\d+)\)/ ) {
        if ( $1 < 10485760 ) {
            $out_type = "varchar($1)"
              ;    # That's just a varchar to us, as these can store up to 1GB
        }
        else {
            $out_type = "varchar";
        }
        ...
        /VARCHAR\s*\((\d+)\s+(?:OCTETS|CODEUNITS16|CODEUNITS32)\s*\)/ )
    {
...
# Also remove minus in object name… they are not supported in PostgreSQl (and in the SQL standard either i think)
    if ( $kw =~ /-/ ) {
        print STDERR
          "I had to rename $kw as it contained a '-' sign. Removing it\n";
        $kw =~ s/-//g;
    }
    ...
    return
      lc($kw);    # FIXME: change this if we want to do a case sensitive schema
}
...
# get rid of newlines, anyway we just want to import the objects (views), they will be reformatted by PG anyway
    ...
    $data =~
      s/(\d{4}-\d{2}-\d{2})-(\d{2}).(\d{2}).(\d{2}).(\d{6})/$1 $2:$3:$4.$5/;
    ...
    # The rest
    $data =~
      s/WITH ROW MOVEMENT|WITH NO ROW MOVEMENT//i;    # No meaning anyway in PG
    return $data;
}
...
my %renames
  ; # Contains all the object names (sequences, tables, indexes…) I had to create to avoid conflicts
...
# First, populate %renames if this is the first call, with the table names (these wont move)
    unless (%renames) {
        foreach my $schema ( keys %{ $schema_db2->{SCHEMAS} } ) {
            foreach my $table (
                keys %{ $schema_db2->{SCHEMAS}->{$schema}->{TABLES} } )
            {
                $renames{ protect_reserved_keywords($schema) }
                  ->{ protect_reserved_keywords($table) } = 'TABLE';
            }
        }
    }
...
#	print STDERR "NEW OBJECT : $schema , $name , $type\n";
    ...
    unless ( exists $renames{$schema}->{ $name . '_' . $type } ) {
        ...
        print STDERR
"I had to rename the $type $schema.$name to $schema.${name}_${type} to avoid conflict\n";
        return $name . '_' . $type;
    }
    my $id = 1;
    ...
    print STDERR
"I had to rename the $type $schema.$name to $schema.${name}${id} to avoid conflict\n";
...
sub find_index_in_schema {
    ...
    foreach my $table ( keys %{ $schema_db2->{SCHEMAS}->{$schema}->{TABLES} } )
    {
        foreach my $index (
            keys %{
                $schema_db2->{SCHEMAS}->{$schema}->{TABLES}->{$table}->{INDEXES}
            }
          )
        {
...
sub parse_dump {
    ...
    # We now ask guess...
    my $decoder =
      guess_encoding( $data_guess, qw/iso8859-15 utf8 utf16-le utf16-be/ );
    die $decoder unless ref($decoder);

    #	print "encoding: " . $decoder->name . "\n";

    open IN, "<:encoding(" . $decoder->name . ")", $filename
    ...
    while ( my $refstatement = read_statement ) {
       ...
       # Special cases for some versions that split create indexes in two lines:
        ...
        if ( $line =~ /^CREATE (UNIQUE )?INDEX/
            and not $line =~ /^CREATE (UNIQUE )?INDEX.*ON "/ )
        {
            $line .= shift(@$refstatement);
        }

        if ( $line =~
/^CREATE (?:REGULAR|LARGE|(?:USER )?TEMPORARY) TABLESPACE "?(.*?)\s?"?($|\s)/
          )
        {
            # Parse tablespace
            my $name = $1;
                    ...
                    /^\s*USING \((?:\s?FILE )?'(.*)'(?:\s?\d+)?\s?(\)|,)/ )
                {
                    ...
                    if ( $2 eq ',' ) {
                        ...
                        while ( my $line = shift(@$refstatement) ) {
                            $line =~ /^\s+(?:FILE )?'(.*)'(?: \d+)?(,|\))/
                              or die
"I don't understand the list of files in this tablespace";
                            push @{ $schema_db2->{TABLESPACE}->{$name}->{PATH}
                            }, ($1);
                            if ( defined $2 ) {
                ...
                next
                  if ( $line =~
/EXTENTSIZE|PAGESIZE|INITIALSIZE|PREFETCHSIZE|BUFFERPOOL|OVERHEAD|TRANSFERRATE|AUTORESIZE|INCREASESIZE|MAXSIZE|FILE SYSTEM CACHING|DROPPED TABLE|USING STOGROUP|DATA TAG/
                  );
            ...
            die( "Overflow in create role: " . join( '', @$refstatement ) )
              unless ( $#$refstatement == -1 );

        }
        elsif ( $line =~ /^COMMENT ON ROLE "(.*?)\s*" IS '(.*?)'?$/ ) {
            die "This role $1 hasn't been seen before"
              unless ( exists $schema_db2->{ROLES}->{$1} );
            $schema_db2->{ROLES}->{$1}->{COMMENT} =
              $2 . "\n" . slurp_comment($refstatement);
            chomp $schema_db2->{ROLES}->{$1}->{COMMENT};
        }
        elsif ( $line =~
/^CREATE SCHEMA "\s*(\S*)\s*"\s*(AUTHORIZATION\s+"\s*(\S*)\s*")?\s*$/
          )
        {
...
# Some roles may be there, and not have been created. I don't know why db2 would do this, but take care of it…
            ...
            die( "Overflow in create schema: " . join( '', @$refstatement ) )
              unless ( $#$refstatement == -1 );
        }
        elsif ( $line =~
/^CREATE SEQUENCE "(.*?)\s*"\."(.*?)\s*" AS (INTEGER|BIGINT|DECIMAL|SMALLINT)/
          )
        {
...
# We don't care about integer/bigint. All sequences are bigint in PG
            ...
            if ( $line =~ /MINVALUE (\d+) MAXVALUE (\d+)/ ) {
                $schema_db2->{SCHEMAS}->{$schema}->{SEQUENCES}->{$sequence}
                  ->{MINVALUE} = $1;
                $schema_db2->{SCHEMAS}->{$schema}->{SEQUENCES}->{$sequence}
                  ->{MAXVALUE} = $2;
            }
            ...
            /^ALTER SEQUENCE "(.*?)\s*"\."(.*?)\s*" RESTART WITH (\d+)\s*$/ )
        {
            $schema_db2->{SCHEMAS}->{$1}->{SEQUENCES}->{$2}->{RESTARTWITH} = $3;
            die( "Overflow in alter sequence: " . join( '', @$refstatement ) )
              unless ( $#$refstatement == -1 );
        }
            ...
            while ( my $line = shift(@$refstatement) ) {
                $colnum++;
                if ( $line =~
/^\s+"(.*?)\s*"\s+(.+?)( NOT NULL)?(?: (?:WITH )?DEFAULT (.*?)| GENERATED (BY DEFAULT|ALWAYS) AS IDENTITY \(| GENERATED (BY DEFAULT|ALWAYS) AS \((.*?)\))?(\s?,| \))?\s*$/
                  )
                {
                    my (
                        $colname,
                        ...
                        $colnotnull,
                        $coldefault,
                        $colgeneratedbydefaultidentity,
                        $colgeneratedbydefaultexpression,
                        $colgeneratedbydefaultexpressionAS,
                        $endofline
                    ) = ( $1, $2, $3, $4, $5, $6, $7, $8 );
                    $schema_db2->{SCHEMAS}->{$schema}->{TABLES}->{$table}
                      ->{COLS}->{$colname}->{TYPE} = convert_type($coltype);
                    $schema_db2->{SCHEMAS}->{$schema}->{TABLES}->{$table}
                      ->{COLS}->{$colname}->{ORIGTYPE} = $coltype;
                    $schema_db2->{SCHEMAS}->{$schema}->{TABLES}->{$table}
                      ->{COLS}->{$colname}->{COLNUM} = $colnum;
                    if ( defined($colnotnull) ) {
                        $schema_db2->{SCHEMAS}->{$schema}->{TABLES}->{$table}
                          ->{COLS}->{$colname}->{NOTNULL} = 1;
                    }
                    else {
                        $schema_db2->{SCHEMAS}->{$schema}->{TABLES}->{$table}
                          ->{COLS}->{$colname}->{NOTNULL} = 0;
                    }
                    if ( defined($coldefault) ) {
                        $schema_db2->{SCHEMAS}->{$schema}->{TABLES}->{$table}
                          ->{COLS}->{$colname}->{DEFAULT} = $4;
                    }
                    ...
                    if ( defined($colgeneratedbydefaultidentity)
                        and not( defined($colgeneratedbydefaultexpression) )
                      ) # Seems there is a bug in certain versions of perl, capturing the GENERATED two times
                    {
                        if ( $colgeneratedbydefaultidentity eq 'BY DEFAULT' ) {
                            $schema_db2->{SCHEMAS}->{$schema}->{TABLES}
                              ->{$table}->{COLS}->{$colname}->{IDENTITY}
                              ->{ALWAYS} = 0;
                        }
                        else {
                            $schema_db2->{SCHEMAS}->{$schema}->{TABLES}
 ...
 # There is no way to emulate the GENERATED ALWAYS. Only warn.
                        ...
                        {
                            print STDERR
"==>Warning: column $colname of table $schema.$table has a default value using GENERATE ALWAYS. This can't be done with PostgreSQL. It will be a default value<==\n";
                        }
                        else {
                            $schema_db2->{SCHEMAS}->{$schema}->{TABLES}
                              ->{$table}->{COLS}->{$colname}->{IDENTITY}
                        ...
                        $schema_db2->{SCHEMAS}->{$schema}->{TABLES}->{$table}
                          ->{COLS}->{$colname}->{DEFAULT} =
                          $colgeneratedbydefaultexpressionAS;
                        print STDERR
"==>Warning: column $colname of table $schema.$table has a default value using an expression. This may not work... you may have to correct this manually, and write a trigger<==\n";
                    }
                }
...
/^\s*(?:IN "(.*?)\s*")? *(?:INDEX IN "(.*?)\s*")? *(?:LONG IN "(.*?)\s*")?\s*;?\s*$/
                  )
                {
                    $schema_db2->{SCHEMAS}->{$schema}->{TABLES}->{$table}
                      ->{TBSTABLE} = $1
...
/^ALTER TABLE "(.*?)\s*"\."(.*?)\s*" ALTER COLUMN "(.*?)\s*" RESTART WITH (\d+)\s*$/
          )
        {
            $schema_db2->{SCHEMAS}->{$1}->{TABLES}->{$2}->{COLS}->{$3}
              ->{IDENTITY}->{STARTWITH} = $4;
        }
        elsif ( $line =~
/^ALTER TABLE "(.*?)\s*"\."(.*?)\s*"\s* DEACTIVATE ROW ACCESS CONTROL$/
          )
        {
            next;    # Just ignore
        }
        elsif ( $line =~ /^ALTER TABLE "(.*?)\s*"\."(.*?)\s*"\s*(\bADD\b.*)?/ )
        {
            ...
            if ( $line =~
                /\bADD(?: CONSTRAINT "(.*?)\s*"\s*)? (PRIMARY KEY|UNIQUE)\s*$/ )
            {
                my %object;
                my $type = $2;
                if ( defined $1 ) {
                ...
                while ( my $line = shift(@$refstatement) ) {
                    if ( $line =~ /^\s+\(?(\S+)(,|\))/ ) {
                        ...
                        die
                          "I don't understand $line in an ALTER TABLE section";
                    }
                }
                if ( $type eq 'PRIMARY KEY' ) {
                    $schema_db2->{SCHEMAS}->{$schema}->{TABLES}->{$table}->{PK}
                      = \%object;
                }
                else {
                    $object{TYPE} = 'UNIQUE';
                ...
                $line =~ /\bADD(?: CONSTRAINT "(.*?)\s*"\s*)? FOREIGN KEY\s*$/ )
            {
                my %object;
                $object{TYPE} = 'FK';
                ...
                $line = shift(@$refstatement);
                $line =~ /^\s+REFERENCES "(.*?)\s*"\."(.*?)\s*"\s*$/
                  or die "I don't understand $line in an ALTER TABLE section";
                $object{FKTABLE}  = $2;
                $object{FKSCHEMA} = $1;
                ...
                while ( my $line = shift(@$refstatement) ) {
                    if ( $line =~
/^\s+(?:ON (DELETE|UPDATE) (RESTRICT|NO ACTION|CASCADE|SET NULL)|(ENFORCED)|(ENABLE QUERY OPTIMIZATION))\s*$/
                      )
                    {
                        if ( defined $3 ) {
                            $object{ENFORCED} = 1;
                        ...
                        die
"I don't understand $line in an ALTER TABLE FOREIGN KEY SECTION";
                    }
                }

                # We got there, the whole FK is parsed. Store it
                push @{ $schema_db2->{SCHEMAS}->{$schema}->{TABLES}->{$table}
                      ->{CONSTRAINTS} }, ( \%object );
            }    # FK
...
/^CREATE (UNIQUE )?INDEX "(.*?)\s*"\."(.*?)\s*"\s+ON "(.*?)\s*"\."(.*?)\s*"\s*$/
          )
        {
            my ( $indexschema, $indexname, $tableschema, $tablename ) =
              ( $2, $3, $4, $5 );

            # We ignore indexschema… it doesn't exist in PostgreSQl anyway
            if ( defined $1 and $1 eq 'UNIQUE ' ) {
                $schema_db2->{SCHEMAS}->{$tableschema}->{TABLES}->{$tablename}
                  ->{INDEXES}->{$indexname}->{UNIQUE} = 1;
            }
            else {
                $schema_db2->{SCHEMAS}->{$tableschema}->{TABLES}->{$tablename}
                  ->{INDEXES}->{$indexname}->{UNIQUE} = 0;
            }
            ...
            while ( my $line = shift(@$refstatement) ) {
                ...
                if ( $line =~ /^\s+\(?(\S+(?:\s+\S+))(,|\);?)/ ) {
                    push @{ $schema_db2->{SCHEMAS}->{$tableschema}->{TABLES}
                          ->{$tablename}->{INDEXES}->{$indexname}->{COLS} },
                      ($1);
                    last if ( $2 eq ')' );    # End of the list of columns
                }
                else {
                    die
"I don't understand $line in a create index. I expected a list of columns";
                }
            }
            ...
            {
                push @{ $schema_db2->{SCHEMAS}->{$tableschema}->{TABLES}
                      ->{$tablename}->{INDEXES}->{$indexname}->{INCLUDECOLS} },
                  ($1);
                if ( $2 eq ',' )    # There are more columns
                {
                    while ( my $line = shift(@$refstatement) ) {
                        if ( $line =~ /^\s+(\S+)\s*(,|\);?)/ ) {
                            push @{ $schema_db2->{SCHEMAS}->{$tableschema}
                                  ->{TABLES}->{$tablename}->{INDEXES}
                                  ->{$indexname}->{INCLUDECOLS} }, ($1);
                            last
                              if ( $2 eq ')' )
                              ;     # End of the list of columns of the include
                        }
                        else {
                            die
"I don't understand $line in create index. I expected a list of columns for an include section";
                        }
                    }
                }
...
# We have finished the INCLUDE. Read another line in case there is something else
            ...
            if ( $line =~ /(?:DIS)?ALLOW REVERSE SCANS/ ) {
                die( "Overflow in comment on column: "
                      . join( '', @$refstatement ) )
                  unless ( $#$refstatement == -1 );
            }
                ...
                print STDERR
"$indexschema.$indexname is clustered. Clustered (or IOT) indexes aren't supported in PostgreSQL. Creating a normal index\n";
                next;    # Ignore PCTFREE
            }
                ...
                next
                  ; # Ignore these: they are the default and can't be turned off anyway
            }
            elsif (
                $line =~ /^\s*COLLECT (SAMPLED )?(DETAILED )?STATISTICS\s*$/ )
            {
...
/^COMMENT ON COLUMN "(.*?)\s*"\."(.*?)\s*"\."(.*?)\s*"\s* IS '(.*?)'?$/
          )
        {
            ...
            if ( exists $schema_db2->{SCHEMAS}->{$1}->{TABLES}->{$2} ) {
                $schema_db2->{SCHEMAS}->{$1}->{TABLES}->{$2}->{COLS}->{$3}
                  ->{COMMENT} = $4 . "\n" . slurp_comment($refstatement);
                chomp $schema_db2->{SCHEMAS}->{$1}->{TABLES}->{$2}->{COLS}
                  ->{$3}->{COMMENT};
            }
            else {
                $schema_db2->{SCHEMAS}->{$1}->{VIEWS}->{$2}->{COLS}->{$3}
                  ->{COMMENT} = $4 . "\n" . slurp_comment($refstatement);
                chomp $schema_db2->{SCHEMAS}->{$1}->{VIEWS}->{$2}->{COLS}->{$3}
                  ->{COMMENT};
            }
        }
        elsif ( $line =~
            /^COMMENT ON TABLE "(.*?)\s*"\."(.*?)\s*"\s* IS '(.*?)'?$/ )
        {
            # Is this really a table, or is it a view (yeah, db2…)
            if ( exists $schema_db2->{SCHEMAS}->{$1}->{TABLES}->{$2} ) {
                $schema_db2->{SCHEMAS}->{$1}->{TABLES}->{$2}->{COMMENT} =
                  $3 . "\n" . slurp_comment($refstatement);
                chomp $schema_db2->{SCHEMAS}->{$1}->{TABLES}->{$2}->{COMMENT};
            }
            else {
                $schema_db2->{SCHEMAS}->{$1}->{VIEWS}->{$2}->{COMMENT} =
                  $3 . "\n" . slurp_comment($refstatement);
                chomp $schema_db2->{SCHEMAS}->{$1}->{VIEWS}->{$2}->{COMMENT};
            }
        }
        elsif ( $line =~
            /^COMMENT ON INDEX "(.*?)\s*"\."(.*?)\s*"\s* IS '(.*?)'?$/ )
        {
            # Resolve the table associated with this index
            my $table = find_index_in_schema( $1, $2 );
            $schema_db2->{SCHEMAS}->{$1}->{TABLES}->{$table}->{INDEXES}->{$2}
              ->{COMMENT} = $3 . "\n" . slurp_comment($refstatement);
            chomp $schema_db2->{SCHEMAS}->{$1}->{TABLES}->{$table}->{INDEXES}
              ->{$2}->{COMMENT};
        }
        elsif (
            $line =~ /^COMMENT ON VIEW "(.*?)\s*"\."(.*?)\s*"\s* IS '(.*?)'?$/ )
        {
            # Resolve the table associated with this index
            $schema_db2->{SCHEMAS}->{$1}->{VIEWS}->{$2}->{COMMENT} =
              $3 . "\n" . slurp_comment($refstatement);
            ...
            $schema_db2->{SCHEMAS}->{$1}->{COMMENT} =
              $2 . "\n" . slurp_comment($refstatement);
        }
        elsif ( $line =~
            /CREATE DISTINCT TYPE "(.*?)\s*"."(.*?)\s*" AS "SYSIBM  ".(.*)/ )
        {
            # Only manage this case for now: this isn't a type, it's a domain
            $schema_db2->{SCHEMAS}->{$1}->{DOMAINS}->{$2}->{BASETYPE} =
              convert_type($3);
        }
...
# SQL is as input by the user, so let's take care about case
            ...
            if ( not defined $leftover ) {    # There is no leftover
                $leftover = '';
            }
            unless ( $schema =~ /^"/ ) {
                $schema = uc($schema);
            ...
            $schema_db2->{SCHEMAS}->{$schema}->{TRIGGERS}->{$trigger}
              ->{CURRENT_SCHEMA} = $current_schema;
            $schema_db2->{SCHEMAS}->{$schema}->{TRIGGERS}->{$trigger}
              ->{CURRENT_PATH} = $current_path;
            $schema_db2->{SCHEMAS}->{$schema}->{TRIGGERS}->{$trigger}
              ->{STATEMENT} = slurp_statement($refstatement);
        }
        elsif ( $line =~
            /^COMMENT ON TRIGGER "(.*?)\s*"\s*\."(.*?)\s*"\s* IS '(.*?)'?$/ )
        {
            $schema_db2->{SCHEMAS}->{$1}->{TRIGGERS}->{$2}->{COMMENT} =
              $3 . "\n" . slurp_comment($refstatement);
            chomp $schema_db2->{SCHEMAS}->{$1}->{TRIGGERS}->{$2}->{COMMENT};
        }
        elsif ( $line =~
            /^CREATE(?: OR REPLACE)? (?:FUNCTION|PROCEDURE) (?:(\S+)\.)?(\S+)/ )
        {
...
# There can be quotes and whatever in these, depending on how the person has created the function
            ...
            $schema_db2->{SCHEMAS}->{$schema}->{FUNCTIONS}->{$function}
              ->{CURRENT_SCHEMA} = $current_schema;
            $schema_db2->{SCHEMAS}->{$schema}->{FUNCTIONS}->{$function}
              ->{CURRENT_PATH} = $current_path;
            $schema_db2->{SCHEMAS}->{$schema}->{FUNCTIONS}->{$function}
            ...
            print STDERR
"==> Alias from sequence $1 to sequence $2 not migrated. Use search_path in PostgreSQL\n";
        }
            ...
            print STDERR
"Ignoring global temporary table $1. There are only local temporary tables in PostgreSQL\n";
            next;
        }
            ...
            print STDERR
"PostgreSQL has no summary table. Closest thing is materialized view. Please create a materialized view matching $line\n";
        }
        else {
...
sub get_coldef {
    ...
    if ( defined($default) ) {
        if ( $default eq '' ) {
            $rv .= ' DEFAULT '
              . find_default_default($type)
              ;    # This is a WITH DEFAULT without anything more
        }
        else {
...
sub produce_schema_files {
    ...
    open BEFORE, '>:utf8', $before_file
      or die "Cannot open $before_file for writing, $!";
    print BEFORE "set client_encoding to UTF8;\n";

    # Do the AFTER file:
    my $after_file = $data_directory . '/' . "after.sql";
    open AFTER, '>:utf8', $after_file
      or die "Cannot open $before_file for writing, $!";
    print AFTER "set client_encoding to UTF8;\n";

    my $unsure_file = $data_directory . '/' . "unsure.sql";
    open UNSURE, '>:utf8', $unsure_file
      or die "Cannot open unsure_file for writing, $!";
    print UNSURE
...
"-- If there is an obvious improvement that can be done to its generation, please file a bug or provide a patch\n";
    ...
    if ($do_tablespaces) {
      ...
      # A tablespace is a path in PG. We take the first location provided by db2
            print BEFORE "CREATE TABLESPACE "
              . protect_reserved_keywords($tablespace)
              . " LOCATION '"
              . $schema_db2->{TABLESPACE}->{$tablespace}->{PATH}->[0] . "';\n";
        }
    }
    ...
    foreach my $role ( keys( %{ $schema_db2->{ROLES} } ) ) {
        ...
        if ( exists( $schema_db2->{ROLES}->{$role}->{COMMENT} ) ) {
            print BEFORE "COMMENT ON ROLE $role IS '"
              . $schema_db2->{ROLES}->{$role}->{COMMENT} . "';\n";
        }
        print BEFORE "\n";
    }
...
# there is no create schema in it. Don't know why it happens though (and don't care :) )
            $authorization = " AUTHORIZATION "
              . $schema_db2->{SCHEMAS}->{$schema}->{AUTHORIZATION};
        }
        unless ( $schema eq '' ) {
            print BEFORE "CREATE SCHEMA "
              . protect_reserved_keywords($schema)
              . $authorization . ";\n\n";
        }
        if ( exists $schema_db2->{SCHEMAS}->{$schema}->{COMMENT} ) {
            print BEFORE "COMMENT ON SCHEMA $schema IS '"
              . $schema_db2->{SCHEMAS}->{$schema}->{COMMENT} . "';\n";
        }
    }
    ...
    foreach my $schema ( keys( %{ $schema_db2->{SCHEMAS} } ) ) {
        foreach my $sequence (
            keys( %{ $schema_db2->{SCHEMAS}->{$schema}->{SEQUENCES} } ) )
        {
            my $sobj =
              $schema_db2->{SCHEMAS}->{$schema}->{SEQUENCES}->{$sequence};
            ...
            if ( exists $sobj->{RESTARTWITH} ) {
                ...
                if ( $sobj->{RESTARTWITH} < $sobj->{MINVALUE} ) {
                    ...
                    print STDERR
                      "==> Sequence $schema.$sequence has RESTARTWITH(",
                      $sobj->{RESTARTWITH}, ") smaller than MINVALUE(",
                      $sobj->{MINVALUE},    ")",
                      ". Initializing to ", $sobj->{MINVALUE}, "\n";
                    $start = $sobj->{MINVALUE};
                }
                print BEFORE "ALTER SEQUENCE ",
                  protect_reserved_keywords($schema), ".",
                  protect_reserved_keywords($seqname),
                  " RESTART WITH ", $start, ";\n";
            }
    ...
    foreach my $schema ( keys( %{ $schema_db2->{SCHEMAS} } ) ) {
        foreach my $domain (
            keys( %{ $schema_db2->{SCHEMAS}->{$schema}->{DOMAINS} } ) )
        {
            print BEFORE "CREATE DOMAIN $domain AS ",
              $schema_db2->{SCHEMAS}->{$schema}->{DOMAINS}->{$domain}
              ->{BASETYPE},
              ";\n";
        }
    }
    ...
    foreach my $schema ( keys( %{ $schema_db2->{SCHEMAS} } ) ) {
        foreach
          my $table ( keys( %{ $schema_db2->{SCHEMAS}->{$schema}->{TABLES} } ) )
        {
            my $tobj = $schema_db2->{SCHEMAS}->{$schema}->{TABLES}->{$table};
            print BEFORE "CREATE TABLE "
              . protect_reserved_keywords($schema) . "."
              . protect_reserved_keywords($table) . " (\n";
            my @cols;
            ...
            {
                push @cols,
                  (     protect_reserved_keywords($col) . ' '
                      . get_coldef( $tobj->{COLS}->{$col} ) );
            }
            ...
            if (
                exists $schema_db2->{SCHEMAS}->{$schema}->{TABLES}->{$table}
                ->{COMMENT} )
            {
                print BEFORE "COMMENT ON TABLE "
                  . protect_reserved_keywords($schema) . "."
                  . protect_reserved_keywords($table) . " IS '"
                  . $tobj->{COMMENT} . "';\n";
            }
            foreach my $col (
                sort {
              ...
              )
            {
                if ( exists $tobj->{COLS}->{$col}->{COMMENT} ) {
                    print BEFORE "COMMENT ON COLUMN "
                      . protect_reserved_keywords($schema) . "."
                      . protect_reserved_keywords($table) . "."
                      . protect_reserved_keywords($col) . " IS '"
                      . $tobj->{COLS}->{$col}->{COMMENT} . "';\n";
                }
            }
            print BEFORE "\n";
    ...
    foreach my $schema ( keys( %{ $schema_db2->{SCHEMAS} } ) ) {
          ...
          my $table ( keys( %{ $schema_db2->{SCHEMAS}->{$schema}->{TABLES} } ) )
        {
            ...
            # Create PK
            if ( exists $tobj->{PK} ) {
                print AFTER "ALTER TABLE " . protect_reserved_keywords($schema),
                  ".", protect_reserved_keywords($table), " ADD";
                if ( exists $tobj->{PK}->{NAME} ) {
                    print AFTER " CONSTRAINT "
                      . check_and_rename( $schema, $tobj->{PK}->{NAME}, 'PK' );
                }
                print AFTER " PRIMARY KEY (",
                  join( ',',
                    protect_reserved_keywords( @{ $tobj->{PK}->{COLS} } ) ),
                  ");\n";
            }
            ...
            foreach my $constraint ( @{ $tobj->{CONSTRAINTS} } ) {
                ...
                if ( exists $constraint->{NAME} ) {
                    print AFTER " CONSTRAINT ",
                      check_and_rename( $schema, $constraint->{NAME},
                        'UNIQUE' );
                }
                print AFTER " UNIQUE (",
                  join( ',',
                    protect_reserved_keywords( @{ $constraint->{COLS} } ) ),
                  ");\n";
            }
        }
    }
...
# covering indexes (patches are under way), change this
                ...
                print AFTER "CREATE ", $iobj->{UNIQUE} ? 'UNIQUE ' : '',
                  "INDEX ",
                  check_and_rename( $schema, $index, 'INDEX' ),
                  ' ON ' . protect_reserved_keywords($schema), '.',
                  protect_reserved_keywords($table),
                  ' (',
...
# If the index isn't unique, we warn that we have added the columns in the main index
                    if ( $iobj->{UNIQUE} ) {
                        ...
                        print AFTER "CREATE INDEX ",
                          check_and_rename( $schema, $index . '_cov1',
                            'INDEX' ),
                          ' ON ' . protect_reserved_keywords($schema), '.',
                          protect_reserved_keywords($table), ' (',
                          join( ',',
                            protect_reserved_keywords( @{ $iobj->{COLS} } ) ),
                          ',',
                          join(
                            ',',
                            protect_reserved_keywords(
                                @{ $iobj->{INCLUDECOLS} }
                            )
                          ),
                          ");\n";
                        print STDERR
"==> IMPORTANT: $index is a UNIQUE covering index. These don't exist yet in PostgreSQL. I replaced it with 2 indexes<==\n";
                    }
                    else {
# We just append these columns. It will only incur a slight slowdown in this index's updates
                        print AFTER ',',
                          ...
                          ");\n";
                        print STDERR
"==> IMPORTANT: $index is a covering index. As it is not unique, INCLUDE columns have been added as plain index columns<==\n";
                    }
                }
                else {
                ...
                if ( exists $iobj->{COMMENT} ) {
                    print AFTER "COMMENT ON INDEX "
                      . protect_reserved_keywords($schema) . "."
                      . protect_reserved_keywords($index) . " IS '"
                      . $iobj->{COMMENT} . "';\n";
                }
            }
        }
    ...
    foreach my $schema ( keys( %{ $schema_db2->{SCHEMAS} } ) ) {
          ...
          my $table ( keys( %{ $schema_db2->{SCHEMAS}->{$schema}->{TABLES} } ) )
        {
            ...
            foreach my $constraint ( @{ $tobj->{CONSTRAINTS} } ) {
                ...
                if ( exists $constraint->{NAME} ) {
                    print AFTER " CONSTRAINT "
                      . protect_reserved_keywords( $constraint->{NAME} );
                }
                print AFTER " FOREIGN KEY (",
                  join( ",",
                    protect_reserved_keywords( @{ $constraint->{LOCALCOLS} } )
                  ),
                  ") REFERENCES ",
                  protect_reserved_keywords( $constraint->{FKSCHEMA} ), '.',
                  protect_reserved_keywords( $constraint->{FKTABLE} ),  " (",
                  join( ",",
                    protect_reserved_keywords( @{ $constraint->{REMOTECOLS} } )
                  ),
                  ") ",
                  "ON DELETE ", $constraint->{ONDELETE}, " ON UPDATE ",
                  $constraint->{ONUPDATE};
                ...
                print UNSURE "ALTER TABLE ",
                  protect_reserved_keywords($schema), ".",
                  protect_reserved_keywords($table),  " VALIDATE CONSTRAINT ";
...
# Do the identities if there are any. Mapped to PostgreSQL's serial (a sequence + a default value)
              ...
              )
            {
                ...
                if ( $iobj->{ORDER} ) {
                    print STDERR
"==> Sequences in PostgreSQL don't have an ORDER restriction, and $schema.$col has an identity that is ORDER. Behavior will be different<==\n";
                }
                if ( $iobj->{ALWAYS} ) {
                    print STDERR
"==> Sequences in PostgreSQL don't have an ALWAYS restriction, and $schema.$col has an identity that is ALWAYS. Behavior will be different<==\n";
                }
                my $seqname =
                  check_and_rename( $schema, $table . "_" . $col . "_seq",
                    'SEQUENCE' );
                print BEFORE "CREATE SEQUENCE ",
                  protect_reserved_keywords($schema), ".",
                  protect_reserved_keywords($seqname),
                  " INCREMENT BY ", $iobj->{INCREMENTBY}, "\n",
                  " MINVALUE ",     $iobj->{MINVALUE},    " MAXVALUE ",
                  $iobj->{MAXVALUE}, "\n",
                  " START ", $iobj->{STARTWITH}, " CACHE ", $iobj->{CACHE},
                  "\n", ' ',
                  $iobj->{CYCLE} ? '' : 'NO', " CYCLE ",
                  "OWNED BY ", protect_reserved_keywords($schema), ".",
                  protect_reserved_keywords($table), ".",
                  protect_reserved_keywords($col),   ";\n";
                print BEFORE "ALTER TABLE ",
                  protect_reserved_keywords($schema), ".",
                  protect_reserved_keywords($table),
                  " ALTER COLUMN ",         protect_reserved_keywords($col),
                  " SET DEFAULT nextval('", protect_reserved_keywords($schema),
                  ".",
                  protect_reserved_keywords($seqname), "');\n";
                print AFTER "SELECT setval('\"",
                  protect_reserved_keywords($schema), "\".\"",
                  protect_reserved_keywords($seqname),
                  "\"',(SELECT max(", protect_reserved_keywords($col),
                  ")::bigint FROM ", protect_reserved_keywords($schema), ".",
                  protect_reserved_keywords($table), "));\n";
            }
        }
    }
...
# FIXME: for now, hope that CURRENT_PATH is like PG's search_path. Ignore CURRENT_SCHEMA
        ...
        print UNSURE "CREATE VIEW "
          . protect_reserved_keywords( $vobj->{SCHEMAS} ), ".",
          protect_reserved_keywords( $vobj->{NAME} ), " ";
...
# Don't forget the comments on views. As it was easier, they aren't stored in the same place (they are in the schema, not in a list)
    foreach my $schema ( keys( %{ $schema_db2->{SCHEMAS} } ) ) {
        foreach
          my $view ( keys( %{ $schema_db2->{SCHEMAS}->{$schema}->{VIEWS} } ) )
        {
            my $vobj = $schema_db2->{SCHEMAS}->{$schema}->{VIEWS}->{$view};

            if ( exists $vobj->{COMMENT} ) {
                print UNSURE "COMMENT ON VIEW ",
                  protect_reserved_keywords($schema), ".",
...
"-- They are only here so that they crash at creation and you notice them and correct them by hand\n";
    foreach my $schema ( keys( %{ $schema_db2->{SCHEMAS} } ) ) {
        ...
        foreach my $function (
            keys %{ $schema_db2->{SCHEMAS}->{$schema}->{FUNCTIONS} } )
        {
            my $funcobj =
              $schema_db2->{SCHEMAS}->{$schema}->{FUNCTIONS}->{$function};
            print UNSURE "CREATE OR REPLACE FUNCTION ",
              protect_reserved_keywords($function),
              " AS\n\$func\$\n",
              $funcobj->{STATEMENT}, "\n\$func\$\n;\n";
        }
    }
...
"-- They are only here so that they crash at creation and you notice them and correct them by hand\n";
    foreach my $schema ( keys( %{ $schema_db2->{SCHEMAS} } ) ) {
        ...
        foreach my $trigger (
            keys %{ $schema_db2->{SCHEMAS}->{$schema}->{TRIGGERS} } )
        {
            my $trigobj =
              $schema_db2->{SCHEMAS}->{$schema}->{TRIGGERS}->{$trigger};
            print UNSURE "CREATE OR REPLACE FUNCTION ",
              protect_reserved_keywords( $trigger . '_fn' ),
...
#FIXME: Should create the trigger, but that's not possible, as the function will fail
            ...
            if ( exists $trigobj->{COMMENT} ) {
                print UNSURE "COMMENT ON $trigger IS '"
                  . $trigobj->{COMMENT} . "';\n";
            }
        }
    }
...
# Store what we need from the catalog, for deltocopy: a list of tables, colnames and types. Make a very simple, tab separated format
    ...
    open TABLEDESC, ">", $tabledescname
      or die "Cannot open $tabledescname for writing, $!";
    foreach my $schema ( sort( keys %{ $schema_db2->{SCHEMAS} } ) ) {
        foreach my $table (
            sort( keys %{ $schema_db2->{SCHEMAS}->{$schema}->{TABLES} } ) )
        {
            my $tobj = $schema_db2->{SCHEMAS}->{$schema}->{TABLES}->{$table};
            ...
            {
                print TABLEDESC "\t$col\t", $tobj->{COLS}->{$col}->{ORIGTYPE},
                  "\t", $tobj->{COLS}->{$col}->{NOTNULL} ? 'NOTNULL' : 'NULL',
                  "\n";
            }
...
#	my $scriptname=$data_directory . '/export.db2' . (is_target_windows()?'.bat':'.sh');
    ...
    open SCRIPT, ">", $scriptname
      or die "Cannot open $scriptname for writing, $!";

    # First connect
    print SCRIPT
      "connect to $db2dbname user $db2username using '$db2password'\n";
    ...
    foreach my $schema ( sort( keys %{ $schema_db2->{SCHEMAS} } ) ) {
        ...
        {
            print SCRIPT
"EXPORT TO ${schema}.${table}.del  of del  LOBS to . MODIFIED BY LOBSINFILE messages ${schema}.${table}.log  SELECT  * FROM \"${schema}\".\"${table}\"\n";
        }
    }
    close SCRIPT;
}
